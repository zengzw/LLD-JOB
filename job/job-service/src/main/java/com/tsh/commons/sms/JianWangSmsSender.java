...
@Service(value="jianWangSmsSender")
public class JianWangSmsSender implements SmsSender {

    private Logger logger = Logger.getLogger(JianWangSmsSender.class);
    ...
    public Result sendPhoneMessage(Result result, String phone, String content)
        ...
        SmsResult se = SmsSendFactory.createSms(TshDiamondClient.getInstance().getConfig("sms_jianwang")).sendSms(map);		
        if (se.isSuccess()) {
